...
//	private static final String ERROR_MSG = "Una excepcion se ha generado en la aplicacion";
	...
	public DgFarmException handleException(Exception e, Logger logger,
		...
		message.append("Disculpe en este momento no es posible realizar esta operaci�n. (");
		message.append(transactionId);
		message.append(")");
		NotHandledException notHandledException = new NotHandledException(
				message.toString());
	...
	/**
	 * metodo que define cuando una excepcion es considerada controlada o no. El
	 * parametro que indica si la excepci�n es controlada es el codigo de error
	 * que se genera como respuesta luego de hacer la invocacion de un PL/SQL
	 * 
	 * @param outParams
	 *            parametros que genera como respuesta la llamada del PL/SQL
	 * @param logger
	 *            el objeto logger
	 * @throws DgFarmException
	 */

	public void handleOutParameterCode(Map<String, Object> outParams,
			Logger logger) throws DgFarmException {
		if (outParams.containsKey("CD_ERROR")
				&& outParams.get("CD_ERROR") != null) {
			if (Integer.parseInt(String.valueOf(outParams.get("CD_ERROR"))) > 0) {
				throw new HandledException(String.valueOf(outParams
						.get("DE_ERROR")));
			}
			if (Integer.parseInt(String.valueOf(outParams.get("CD_ERROR"))) < 0) {
				throw new NotHandledException(String.valueOf(outParams
						.get("DE_ERROR")));
			}
		}
	}
	...
	/**
	 * <p>
	 * funcion que hace la escritura en el log de la aplicacion del valor
	 * retornado por la funcion PL/SQL luego de haber hecho la evaluacion
	 * 
	 * </p>
	 * 
	 ...
	 * 
	 * @param params
	 *            <code>Object;</code> objeto a ser escrito en el log usados
	 *            para invocar el stored procedure.
	 * @param logger
	 ...
	 */
	public void writeOutParamTraceFunction(Object params, Logger logger,
			String... args) {
		...
		if (logger.isEnabled(Level.INFO)) {
			logger.log(
					Level.INFO,
					new String[] { transaccion, aplicacion, origen, metodo,
							plataforma, dataSource, procedure,
							inParams.toString(), time });
		}
	}

	/**
	 * <p>
	 * Transforma un <code>Object</code> en un <code>String</code>.
	 * </p>
	 * 
	 * .
	 * 
...
//				value = ((java.util.Collection) params).size() + " registros";
//			} else {
//
//				value = params;
//			}
...
//
//		if (params != null) {
//			result.append(CommonsDao.OPEN_BRACKET)
//					.append(params.getClass().getSimpleName())
//					.append(CommonsDao.CLOSE_BRACKET);
//		} else {
//			result.append(CommonsDao.OPEN_BRACKET).append("null")
//					.append(CommonsDao.CLOSE_BRACKET);
//		}
//
...
//	}
	...
	public void writeOutParamTrace(Map<String, Object> params, Logger logger,
			String... args) {
		String transaccion = args[0];
		String aplicacion = args[1];
		String origen = args[2];
		...
		if (logger.isEnabled(Level.INFO)) {
			logger.log(
					Level.INFO,
					Arrays.toString(new String[] { transaccion, aplicacion,
							origen, metodo, plataforma, dataSource, procedure,
							inParams.toString(), time }));
		}
	...
	public void writeInParamTrace(Map<String, Object> inParams, Logger logger,
			String... args) {
	 ...
	 * @param params
	 *            <code>Map&lt;String, Object&gt;</code> par&aacute;metros
	 *            usados para generar la excepci&oacute;n de dgfarm y
	 *            escribir en el log.
	 * @param logger
	 *            <code>Logger</code> logger de la aplicaci&oacute;n.
	 * @param args
	 *            <code>String...</code> argumentos adicionales para su
	 *            escritura en el log de la aplicaci&oacute;n.
	 * @return excepci&oacute;n de dgfarm.
	 */
	public Exception handleException(Exception e, Map<String, Object> params,
	...
	public void writeDgFarmExceptionTrace(final Logger logger,
		...
		if (scex instanceof HandledException) {
			if (logger.isEnabled(Level.WARN)) {
				logger.log(
						Level.WARN,
						new String[] { scex.getTransaccion(),
	...
	public String getOrigen() {
		return (new StringBuffer(this.getClass().getCanonicalName()))
				.toString();
	}

	public String getOrigen(String method) {
		return (new StringBuffer(getOrigen()).append(".").append(method))
				.toString();
	}
